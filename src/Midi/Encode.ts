...
import type { KeySignatureEvent, MetaEvent, TempoEvent, TimeSignatureEvent } from './Header'
...
function encodeNote(
  note: Note,
  channel: number,
): [MidiNoteOnEvent & WithAbsoluteTime, MidiNoteOffEvent & WithAbsoluteTime] {
  return [
    {
...
function encodePitchBend(pb: PitchBend, channel: number): MidiPitchBendEvent & WithAbsoluteTime {
  return {
...
function encodeInstrument(track: Track): (MidiProgramChangeEvent & WithAbsoluteTime)[] {
  const x: (MidiProgramChangeEvent & WithAbsoluteTime)[] = [
    {
      absoluteTime: 0,
...
function encodeTimeSignature(
  timeSig: TimeSignatureEvent,
): MidiTimeSignatureEvent & WithAbsoluteTime {
  return {
...
function encodeText(textEvent: MetaEvent & { ticks: number }): MidiTextEvent & WithAbsoluteTime {
  return {
