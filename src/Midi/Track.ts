...
import type { ControlChangeJSON } from './ControlChange'
import { type ControlChangesJSON, createControlChanges } from './ControlChanges'
import { PitchBend, type PitchBendInterface, type PitchBendJSON } from './PitchBend'

import { Header } from './Header'
import { Instrument, type InstrumentJSON } from './Instrument'
import { Note, type NoteConstructorInterface, type NoteJSON } from './Note'
...
export class Track {
  ...
  constructor(trackData: MidiEvent[], header: Header) {
    ...
    if (trackData) {
      ...
      const endOfTrackEvent: (MidiEndOfTrackEvent & WithAbsoluteTime) | undefined = trackData.find(
        (event): event is MidiEndOfTrackEvent & WithAbsoluteTime => event.type === 'endOfTrack',
      )

      this.endOfTrackTicks =
        endOfTrackEvent !== undefined ? endOfTrackEvent.absoluteTime : undefined
    }
  }
  ...
  get durationTicks(): number {
    ...
    let maxDuration =
      this.notes[this.notes.length - 1].ticks + this.notes[this.notes.length - 1].durationTicks
    for (let i = 0; i < this.notes.length - 1; i++) {
